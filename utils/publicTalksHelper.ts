// utils/publicTalksHelper.ts

export const PUBLIC_TALK_THEMES: { [key: number]: string } = {
...
export const REVERSE_PUBLIC_TALK_THEMES: { [key: string]: number } = Object.fromEntries(
    Object.entries(PUBLIC_TALK_THEMES).map(([num, theme]) => [theme, parseInt(num, 10)])
);


export const getThemeByOutlineNumber = (numStr: string): { theme: string; warning?: string } => {
    const num = parseInt(numStr, 10);
    if (isNaN(num) || !numStr) {
        return { theme: '' };
    }
    
    const theme = PUBLIC_TALK_THEMES[num];

    if (!theme) {
        return { theme: 'Número de esboço inválido' };
    }

    if (theme === '(Não use.)') {
        return { theme: 'Este esboço não deve ser usado', warning: 'Este esboço não deve ser usado' };
    }
