    ...
    if (!schedule) {
        return (
             <div className="relative block p-6 bg-white/70 dark:bg-slate-800/60 backdrop-blur-xl rounded-3xl shadow-lg border border-white/50 dark:border-slate-700/50 h-full min-h-[224px] flex flex-col justify-between">
                <div>
                    <div className="flex items-center justify-center h-12 w-12 rounded-xl bg-teal-500">
                        <PodiumIcon className="h-7 w-7 text-white" />
                    </div>
                </div>
                <div>
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white">Discurso Público</h3>
    ...
    return (
        ...
        >
            <div>
                <div className="flex justify-between items-start">
                    ...
                    <div className="text-right">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white">Discurso Público</h3>
                        <p className="text-sm font-semibold text-teal-600 dark:text-teal-400">{formattedDate}</p>
                    </div>
                </div>
                ...
                <div className="mt-4 space-y-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-xs uppercase text-slate-500 dark:text-slate-400 font-semibold">Tema</p>
                            <p className="font-semibold text-slate-800 dark:text-slate-200">{schedule.theme}</p>
                        </div>
                    </div>
                     <div className="flex items-center gap-3">
                        <ConductorIcon className="h-6 w-6 text-slate-400 dark:text-slate-500 flex-shrink-0" />
                         <div>
                            <p className="text-xs uppercase text-slate-500 dark:text-slate-400 font-semibold">Orador</p>
                            <p className="font-semibold text-slate-800 dark:text-slate-200">{schedule.speakerName} - {schedule.congregation}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex justify-end items-center mt-2">
                 <span className="text-sm font-semibold text-primary dark:text-blue-400">Ver detalhes</span>
