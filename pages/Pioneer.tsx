...
const Pioneer: React.FC = () => {
  ...
  const [editingActivity, setEditingActivity] =
    useState<PioneerActivity | null>(null);
  const [isDeleteConfirmOpen, setIsDeleteConfirmOpen] = useState(false);
  const [activityToDelete, setActivityToDelete] = useState<string | null>(null);
  const [isReportOverwriteConfirmOpen, setIsReportOverwriteConfirmOpen] =
    useState(false);
  const [shouldShareOnConfirm, setShouldShareOnConfirm] = useState(true);
  const [isNameModalOpen, setIsNameModalOpen] = useState(false);
  const [tempPublisherName, setTempPublisherName] = useState("");
  const [tempPublisherGroup, setTempPublisherGroup] = useState<"1" | "2" | "3">(
    "1",
  );
  ...
  const currentRecord = useMemo(() => {
    return records.find(
      (r) => r.month === selectedMonth && r.createdBy === user?.uid,
    );
  ...
  const totalRevisits = useMemo(() => {
    if (!currentRecord) return 0;
    return currentRecord.activities.reduce(
      (acc, act) => acc + (act.revisits || 0),
      0,
    );
  }, [currentRecord]);
  ...
  const handleRoleChange = async (
    newRole: "Publicador" | "Pioneiro Auxiliar" | "Pioneiro Regular",
  ) => {
    if (!user || !profile) return;
    setLoading(true);
    try {
      const defaultGoal =
        newRole === "Pioneiro Regular"
          ? 50
          : newRole === "Pioneiro Auxiliar"
            ? 30
            : 0;
  ...
  const handleAddOrUpdateActivity = async (activity: PioneerActivity) => {
    ...
    try {
      ...
      let targetRecord = records.find(
        (r) => r.month === monthStr && r.createdBy === user.uid,
      );

      if (!targetRecord) {
        const defaultGoal =
          currentRole === "Pioneiro Regular"
            ? 50
            : currentRole === "Pioneiro Auxiliar"
              ? 30
              : 0;
        ...
        if (editingActivity) {
          ...
          if (oldMonthStr === monthStr) {
            ...
            const oldRecord = records.find(
              (r) => r.month === oldMonthStr && r.createdBy === user.uid,
            );
            if (oldRecord) {
              const filteredOld = oldRecord.activities.filter(
                (a) => a.id !== editingActivity.id,
              );
              await updatePioneerRecord(
                oldRecord.id,
                { activities: filteredOld },
                user.uid,
              );
            }
  ...
  const confirmDeleteActivity = async () => {
    ...
    try {
      const updatedActivities = currentRecord.activities.filter(
  ...
  const handleSubmitMonthlyReport = async (shouldShare: boolean = true) => {
    if (!user) return;

    setShouldShareOnConfirm(shouldShare);
    ...
    await processReportSubmission(undefined, shouldShare);
  };
  ...
  const handleConfirmName = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const [recordsData] = await Promise.all([getPioneerRecords()]);
      const userRecords = recordsData.filter((r) => r.createdBy === user.uid);
      setRecords(userRecords);
      ...
      await processReportSubmission(updatedProfile, shouldShareOnConfirm);
    } catch (error) {
  ...
  const processReportSubmission = async (
    activeProfile?: PublisherProfile,
    ...
    try {
      ...
      await addReport(
        {
          ...
          notes:
            reportNotes ||
            `Relatório enviado pelo App de Relatório de Serviço.`,
          isActive: true,
        },
        user.uid,
      );
      ...
      const pioneerRecord: PioneerRecord = {
        ...
        goalHours:
          ...
          (isPioneer ? (currentRole === "Pioneiro Regular" ? 50 : 30) : 0),
        role: currentRole as any,
      ...
      if (shouldShare) {
        ...
        setToastMessage(
          "Relatório gravado e enviado para a aba secretário com sucesso!",
        );
        ...
        if (navigator.share) {
          ...
          try {
            ...
            setToastMessage(
              "Relatório enviado com sucesso e copiado para compartilhar!",
            );
        ...
        setToastMessage("Relatório gravado e enviado com sucesso!");
      }
  ...
  const generateMonthlyPDF = () => {
    ...
    doc.setFontSize(18);
    doc.text(`Relatório de Serviço - ${monthName} / ${year}`, 105, 20, {
      align: "center",
    });
    ...
    if (currentRecord?.activities && currentRecord.activities.length > 0) {
      ...
      let y = 101;
      const sortedActivities = [...currentRecord.activities].sort((a, b) =>
  ...
  const historyChartData = useMemo(() => {
    return MONTHS.map((name, i) => {
      ...
      let hours = 0;
      if (rec) {
        const totalMinutes = rec.activities.reduce(
          (acc, act) => acc + act.hours * 60 + act.minutes,
          0,
        );
  ...
  return (
    <div
      ...
      className="container mx-auto px-4 sm:px-6 md:px-8 py-6 max-w-4xl pb-32"
    >
      ...
      <section className="bg-slate-200 dark:bg-slate-900 p-1.5 rounded-2xl flex relative mb-6 border border-slate-300 dark:border-slate-800 transition-colors">
        {(["Publicador", "Pioneiro Auxiliar", "Pioneiro Regular"] as const).map(
          (role) => (
            <button
              key={role}
      ...
      <div className="flex items-center justify-between border-b border-slate-300 dark:border-slate-800 pb-5 mb-8 transition-all">
        ...
        <div className="text-center">
          ...
          <p className="text-[10px] font-extrabold text-slate-600 dark:text-slate-300 uppercase tracking-widest mt-0.5">
            Mês de Referência
          </p>
        </div>
        <button
                ...
                <div className="bg-emerald-500/10 border border-emerald-500/20 p-5 rounded-3xl text-center space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-700 dark:text-slate-300 mt-1">
                      Sua atividade mensal foi enviada com sucesso ao
                      Secretário.
                    </p>
                  </div>
                  ...
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 text-left grid grid-cols-2 gap-3 divide-x divide-slate-200 dark:divide-slate-800">
                    <div className="text-center">
                      <p className="text-[10px] font-extrabold text-slate-600 dark:text-slate-300 uppercase">
                        Pregação
                      </p>
                      <p className="text-lg font-black text-slate-900 dark:text-white">
                        {participated ? "Participei" : "Não Participei"}
                      </p>
                    </div>
                    <div className="text-center">
                      <p className="text-[10px] font-extrabold text-slate-600 dark:text-slate-300 uppercase">
                ...
                <div className="space-y-8">
                  <div className="border-b border-slate-100 dark:border-white/5 pb-4">
                    <h3 className="text-md font-extrabold text-slate-800 dark:text-white">
                      Enviar Fechamento Mensal
                    </h3>
                    <p className="text-xs text-slate-400 mt-1">
                      Preencha os dados simples de serviço para este mês.
                    </p>
                  </div>

                  {/* Participated Field */}
                  <div className="space-y-4">
                    <label className="block text-xs font-black text-slate-700 dark:text-slate-200 uppercase tracking-widest">
                      Participou no serviço este mês?
                    </label>
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900/55 p-4 rounded-2xl">
                          ...
                          <div className="flex items-center gap-3">
                            <button
                              onClick={() =>
                                setReportStudies(Math.max(0, reportStudies - 1))
                              }
                            ...
                            <button
                              onClick={() =>
                                setReportStudies(reportStudies + 1)
                              }
                              className="h-10 w-10 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-white/5 font-black text-slate-600 dark:text-slate-300 hover:bg-slate-100 transition-colors flex items-center justify-center"
                            >
                              +
                        ...
                        <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900/55 p-4 rounded-2xl">
                          <div>
                            ...
                            <p className="text-[10px] text-slate-400">
                              Tempo dedicado à pregação
                            </p>
                          </div>
                          <div className="flex items-center gap-3">
                            <button
                              onClick={() =>
                                setReportHours(Math.max(0, reportHours - 1))
                              }
                              className="h-10 w-10 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-white/5 font-black text-slate-600 dark:text-slate-300 hover:bg-slate-100 transition-colors flex items-center justify-center"
                            >
                              -
                  ...
                  {/* Two submit CTA options */}
                  <div className="flex flex-col sm:flex-row gap-3">
                    <button
                      onClick={() => handleSubmitMonthlyReport(false)}
                      className="flex-1 py-4 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800/80 dark:hover:bg-slate-700/80 text-slate-800 dark:text-white font-black rounded-2xl transition-all flex items-center justify-center gap-2 border border-slate-200 dark:border-slate-700"
                    >
                      <CheckIcon className="h-5 w-5 text-emerald-500" /> Apenas
                      Salvar
                    </button>
                    <button
                      onClick={() => handleSubmitMonthlyReport(true)}
                      className="flex-1 py-4 bg-slate-900 hover:bg-black dark:bg-emerald-600 dark:hover:bg-emerald-500 text-white font-black rounded-2xl shadow-xl transition-all flex items-center justify-center gap-2"
                    >
                      <ShareIcon className="h-5 w-5" /> Salvar & Compartilhar
                    </button>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-6 py-4 transition-all">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mt-0.5">
                        Realizadas de {pioneerMetrics.goal}h
                      </p>
                    </div>
                  ...
                  <div className="relative">
                    ...
                    <div className="flex justify-between text-[10px] text-slate-400 font-bold mt-2">
                      <span>
                        {pioneerMetrics.percent.toFixed(0)}% Concluído
                      </span>
                      <span>
                        Faltam {pioneerMetrics.remaining.toFixed(1)} horas
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 border-t border-slate-100 dark:border-white/5 pt-4">
                    <div className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-2xl text-center">
                      ...
                      <span className="text-lg font-black text-slate-800 dark:text-white mt-1 block">
                        {pioneerMetrics.remaining.toFixed(1)}h
                      </span>
                    </div>
                    <div className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-2xl text-center">
                      <p className="text-xs text-slate-400 font-extrabold tracking-wider uppercase">
                      ...
                      <span className="text-lg font-black text-slate-800 dark:text-white mt-1 block">
                        {pioneerMetrics.avgDaily}h / dia
                      </span>
                    </div>
                  </div>

                  <p className="text-[10px] text-center text-slate-400 italic">
                    Previsão baseada em {pioneerMetrics.daysLeft} dias restantes
                    neste mês.
                  </p>
                </div>
              )}
                            ...
                            >
                              <div className="flex items-center gap-3">
                                <div className="h-10 w-10 bg-slate-100 dark:bg-slate-905 text-slate-600 dark:text-slate-300 rounded-xl flex flex-col items-center justify-center">
                                  <span className="text-[9px] font-black leading-none uppercase text-slate-400">
                                    {d.toLocaleString("pt-BR", {
                                      weekday: "short",
                                    })}
                                  </span>
                                  <span className="text-md font-extrabold leading-none mt-1">
                                    {d.getDate()}
                                  </span>
                                </div>
                                <div>
                                  ...
                                  <span className="text-[10px] bg-slate-100 dark:bg-white/5 px-2 py-0.5 rounded text-slate-500 mt-1 inline-block">
                                    {act.category || "Pregação"}
                                  </span>
                                </div>
                              </div>
                              <div className="flex gap-2">
                    ...
                    <div className="bg-white dark:bg-slate-950 border border-slate-200/50 dark:border-white/5 p-12 text-center rounded-3xl space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400 max-w-xs mx-auto mt-1">
                          Lançar sua atividade diária ajuda você a acompanhar
                          sua meta de pioneiro facilmente.
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="py-6 transition-all">
                    <div className="flex items-center justify-between mb-4">
                      <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                        <ChartBarIcon className="h-4 w-4" /> Evolução Mensal (
                        {selectedYear})
                      </h4>
                    </div>
                ...
                <div className="space-y-6">
                  {currentRecord?.submitted ? (
                    <div className="bg-emerald-500/10 border border-emerald-500/20 p-5 rounded-3xl text-center space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                          Seu relatório de pioneiro foi gravado e compartilhado
                          com sucesso.
                        </p>
                      </div>
                    ...
                    <div className="space-y-6 py-4">
                      <div>
                        <h3 className="text-md font-extrabold text-slate-800 dark:text-white">
                          Fechamento Mensal de Pioneiro
                        </h3>
                        <p className="text-xs text-slate-400 mt-1">
                          Revise os números gerados automaticamente do seu
                          diário de bordo.
                        </p>
                      </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-900/55 p-4 rounded-2xl flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-[10px] text-slate-400">
                              Puxadas automaticamente do diário de bordo
                            </p>
                          </div>
                          <input
                            type="number"
                            value={reportHours}
                            onChange={(e) =>
                              setReportHours(parseFloat(e.target.value) || 0)
                            }
                        ...
                        <div className="bg-slate-50 dark:bg-slate-900/55 p-4 rounded-2xl flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-[10px] text-slate-400">
                              Dirigidos durante o mês inteiro
                            </p>
                          </div>
                          <input
                            type="number"
                            value={reportStudies}
                            onChange={(e) =>
                              setReportStudies(parseInt(e.target.value) || 0)
                            }
                            className="w-20 p-2 text-center text-md font-black bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-white/5 text-slate-800 dark:text-white"
                          />
                        </div>
                        ...
                        <div className="bg-slate-50 dark:bg-slate-900/55 p-4 rounded-2xl flex items-center justify-between">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setReportRevisits(parseInt(e.target.value) || 0)
                            }
                            className="w-20 p-2 text-center text-md font-black bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-white/5 text-slate-800 dark:text-white"
                          />
                        </div>
                      ...
                      <div className="flex flex-col gap-2">
                        <div className="flex gap-2">
                          ...
                          <button
                            onClick={() => handleSubmitMonthlyReport(false)}
                            className="flex-1 py-4 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800/80 dark:hover:bg-slate-700/80 text-slate-800 dark:text-white border border-slate-200 dark:border-slate-700 font-black rounded-2xl transition-all flex items-center justify-center gap-2"
                          >
                            <CheckIcon className="h-5 w-5 text-emerald-500" /> Apenas Salvar
                          </button>
                        </div>
                        <button
                          onClick={() => handleSubmitMonthlyReport(true)}
      ...
      <ConfirmationModal
        ...
        onConfirm={() =>
          processReportSubmission(undefined, shouldShareOnConfirm)
        }
        ...
        {isNameModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <h3 className="text-xl font-black text-slate-800 dark:text-white mb-2 font-outfit">
                Quem está enviando?
              </h3>
              <p className="text-slate-500 dark:text-slate-400 text-xs mb-6 font-sans">
                Identifique seu nome e grupo de serviço de campo para que o
                Secretário possa arquivar corretamente nas fichas da
                congregação.
              </p>
              ...
              >
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 font-sans">
                    Nome Completo
                  </label>
                  <input
                    type="text"
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setTempPublisherGroup(e.target.value as "1" | "2" | "3")
                    }
        ...
        {isGoalModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <h3 className="text-xl font-black text-slate-800 dark:text-white mb-2 font-outfit">
                Definir Alvo de Horas
              </h3>
              <p className="text-slate-500 dark:text-slate-400 text-xs mb-6 font-sans">
                Ajuste a sua meta de horas mensal para acompanhar seu progresso
                e média diária.
              </p>
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  if (tempGoalHours >= 0) {
                    ...
                    setToastMessage(
                      "Por favor, insira um valor válido de horas.",
                    );
                  }
                }}
                className="space-y-4 font-sans"
...
const ActivityModal: React.FC<{
  ...
  const [category, setCategory] = useState<"Pregação" | "Estudos" | "Outra">(
    "Pregação",
  );
          ...
          <div className="grid grid-cols-2 gap-3 pt-2">
            <div>
              ...
              <div className="flex items-center bg-slate-50 dark:bg-slate-800 rounded-xl overflow-hidden px-1 border border-slate-205 dark:border-white/5">
                <button
                  type="button"
                  onClick={() => setRevisits(Math.max(0, revisits - 1))}
                  className="p-2 text-slate-500 font-bold"
                >
                  -
                </button>
                ...
                <button
                  type="button"
                  onClick={() => setRevisits(revisits + 1)}
                  className="p-2 text-slate-500 font-bold"
                >
                  +
                </button>
